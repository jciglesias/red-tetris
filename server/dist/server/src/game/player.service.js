...
let PlayerService = class PlayerService {
    createPlayer(name, isHost = false) {
        return {
            id: (0, uuid_1.v4)(),
            name,
            state: shared_1.PlayerState.WAITING,
            board: (0, shared_1.createEmptyBoard)(),
            spectrum: Array(shared_1.BOARD_WIDTH).fill(0),
            currentPiece: null,
            nextPiece: null,
            isHost,
            linesCleared: 0,
    ...
    updatePlayerBoard(player, board) {
        return {
            ...player,
            board,
            spectrum: (0, shared_1.calculateSpectrum)(board),
        };
    }
    ...
    placePieceOnBoard(player, piece) {
        ...
        piece.shape.forEach(block => {
            ...
            if (x >= 0 && x < shared_1.BOARD_WIDTH && y >= 0 && y < shared_1.BOARD_HEIGHT) {
                newBoard[y][x] = piece.type;
            }
        });
        const completedLines = [];
        for (let y = 0; y < shared_1.BOARD_HEIGHT; y++) {
        ...
        completedLines.reverse().forEach(lineIndex => {
            ...
            newBoard.unshift(Array(shared_1.BOARD_WIDTH).fill(null));
        });
        return {
            board: newBoard,
    ...
    addPenaltyLines(player, lineCount) {
        ...
        for (let i = 0; i < lineCount; i++) {
            const penaltyLine = Array(shared_1.BOARD_WIDTH).fill('penalty');
            const gapIndex = Math.floor(Math.random() * shared_1.BOARD_WIDTH);
        ...
        return {
            ...player,
            board: newBoard,
            spectrum: (0, shared_1.calculateSpectrum)(newBoard),
        };
    }
